...
Unknown ids return `404 not found`. The bare `/status` path keeps returning `packager-status:running` from the React Native community middleware.
...
2. Make code changes
3. Call `get_build_events` again to wait for `bundle_build_done` or `bundle_build_failed` — or poll `GET /status/<id>` for a single known bundler
