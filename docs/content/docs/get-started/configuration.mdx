...
<Callout type="info">
  Using static values (`boolean`, `string`, or `RegExp`) is significantly more performant than functions. Prefer static patterns when possible.
</Callout>
...
<Callout type="warn">
  This option is **dangerous** and can break the build. Use only when you know exactly what you're doing.
</Callout>
