...
server.on('device.connected', ({ client }) => {
  console.log('Device connected:', client.id);
});

server.on('device.disconnected', ({ client }) => {
  console.log('Device disconnected:', client.id);
});

server.on('device.message', ({ client, data }) => {
  console.log('Message from device:', client.id, data);
});

server.on('device.error', ({ client, error }) => {
  console.error('Device error:', client.id, error);
});
