...
describe('rollipop/jest — RN integration', () => {
  ...
  it('rewrites import.meta.env.* to process.env.* so CJS runs cleanly', () => {
    // The transformer rewrites `import.meta.env` to `process.env` at
    // compile time. Access to an undefined key yields `undefined` — if
    // the rewrite failed, the reference to `import.meta` itself would
    // throw `SyntaxError` under jest's CJS loader before this assertion
    // could run.
    expect(import.meta.env.ROLLIPOP_NONEXISTENT).toBeUndefined();
  });
